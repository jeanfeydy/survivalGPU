...
├── scripts/
│   ├── knit_r_pages.R   # knits R/vignettes/*.Rmd.orig -> doc/source/r/user_guide/*.md
...
This skips `doc/scripts/knit_r_pages.R` and `doc/scripts/build_pkgdown.R`, so
...
- **R** (`r/user_guide/*.md`): Sphinx never executes R. Instead,
  `doc/scripts/knit_r_pages.R` knits the *same* live-chunk sources already
...
- **R API reference** (`r/reference/`): built separately by `pkgdown` (using
  ...
  output at `doc/_build/html/r/reference/`. This step must run *after*
