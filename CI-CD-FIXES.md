...
|---|---------|------------|--------------|
| 8 | `Python tests` green on 3.10/3.11 but **`No module named 'rpy2'` on 3.12** (despite "Successfully installed rpy2") | `rpy2` 3.6.x is a *meta-package*; the real code lives in `rpy2-rinterface`/`rpy2-robjects`, which are a PEP 420 **namespace** package (no `rpy2/__init__.py`) and ship **no Linux wheels**. On 3.12 they build from sdist and the `rpy2` namespace fails to register → unimportable. | **Pinned `rpy2<3.6`** (the monolithic 3.5.x line ships `rpy2/__init__.py`, no namespace split) and matched it with **R 4.4.3** (3.5.x doesn't need R 4.5's `R_getVar`). `import rpy2` now works on 3.10/3.11/**3.12**, so 3.12 is back in the tests matrix. |
