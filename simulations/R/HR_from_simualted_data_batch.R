...
source("experiment_parameters_batch.R")
...
experiment_number_list = experiment_parameters$experiment_number_list
...
combinaisons_parameters <- expand.grid(HR_target = HR_target_list,weight_function = weight_functions_list, n_patients = n_patients_list,binarization_dose_list = binarization_dose_list,n_knots= n_knots_list, experiment_number = experiment_number_list)
...
for (i in 1:nrow(combinaisons_parameters)){
    ...
    experiment_number <- combinaisons_parameters$experiment_number[i]
