...
Plot scaled Schoenfeld residuals against the time for each covariates with `ggcoxzph()` function from `survminer` package.
...
Also with the `survminer` package, you can use `ggforest()` function to plot forestplot for Cox PH model. The confidence interval plotted is the confidence interval by the normal distribution (not bootstrap)
...
Is it possible to check Kaplan-Meier survival estimates with `survival::survfit()` function. After you can use `survminer::ggsurvplot()` to plot a survival curve.
