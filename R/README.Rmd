...
survivalGPU is a package based on a package written in python, dependent on the `reticulate` R package. To use it, it's necessary to have installed some python libraries such as `torch`, `torch-scatter`, and `pykeops`. To use survivalGPU, you can create a virtual python environment through `reticulate`. It's highly recommended to not to use the default python executable.
