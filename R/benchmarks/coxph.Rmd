...
list_df_bootstrap <- function(dataset,n_bootstrap){
  ...
  liste_df<-lapply(1:n_bootstrap, function(x) dataset  %>%
...
time_cox <- function(dataset,n_bootstrap){
  ...
  if(n_bootstrap > 100){
    ...
    list_df<-list_df_bootstrap(dataset = dataset,
                               n_bootstrap = n_bootstrap)
    
    start_time<-Sys.time()
    lapply(list_df, function(x) coxph(Surv(Start,Stop,Event) ~ sex + age,
                                      data = x,
                                      id = Id))
...
combine_time<-function(dataset,n_bootstrap){
  ...
  return(list(coxph = time_cox(dataset,n_bootstrap),
...
n_bootstrap <- c(1, 2, 5, 10, 20, 100, 1000, 2000, 10000, 50000)
