...
survivalGPU is a package based on a package written in python, dependent on the `reticulate` R package. To use it, it's necessary to have installed some python libraries such as `torch` in your python environment. `pykeops` is only required if you want to use `wceGPU()` (the WCE model) -- it is not available on Windows, so `coxphGPU()` works without it.

Maybe it's installed in your python default system, but it's better if your create your own virtualenv for your project. Let's make the steps to configure properly a virtual python environment to use `survivalGPU` in R.
