...
wceGPU.default<- function(data, nknots, cutoff, constrained = FALSE,
  ...
  wce<-wce_R(data=data, ids=id, covars=py_covariates, stop=stop,
  ...
  BIC = sapply(wce$loglik,BIC_for_wce,n.events=nevents, n.knots=nknots,
               cons=constrained, aic=aic, covariates=covariates)

  # List to return
  results<-list(knotsmat = knotsmat,
                WCEmat = WCEmat,
                loglik = c(wce$loglik),
                coef = coef,
                vcovmat = vcovmat,
...
print.wceGPU <- function(x, ...){
  ...
  if(object$constrained == FALSE){
    cat_constrained = "Unconstrained"}else if(object$constrained %in% c("R","r","RIGHT","Right","right")){
      cat_constrained = "Right constrained"}else if(object$constrained %in% c("L","l","LEFT","Left","left")){
        cat_constrained = "Left constrained"}else{
          cat_constrained = "Constrained ?"
        }

  cat(paste("------- ",cat_constrained,"model, with",object$nknots,
  ...
  cat(paste("Number of events :",object$nevents[1]),
      ...
      paste(ifelse(object$aic==TRUE,"AIC :","BIC :"),
  ...
  if(object$nbootstraps > 1){
    ...
    cat("\n ---------------- \n")
    cat(paste0("With bootstrap (", object$nbootstraps,
...
summary.wceGPU <- function(object, ...){
  ...
  coef_mat <- data.frame(coef = estimates,
                         ci_inf = conf.int[,1],
                         ci_sup = conf.int[,2],
  ...
  colnames(coef_mat) <- c("coef",paste("CI",colnames(conf.int)),"exp(coef)",
                          "se(coef)","z","p")
...
coef.wceGPU <- function(object, ...){
  ...
  if(is.null(object$covariates)){
    ...
    if(object$nbootstraps == 1){
      list(WCEest = object$coef[,!colnames(object$coef) %in% object$covariates],
           covariates = object$coef[,object$covariates])
    }else{
      list(coef = list(WCEest = object$coef[,!colnames(object$coef) %in% object$covariates],
                       covariates = object$coef[,object$covariates]),
           CI = list(WCEest = object$coef_CI[,!colnames(object$coef) %in% object$covariates],
...
plot.wceGPU <- function(x, ..., hist.covariates = FALSE){
  ...
  if(object$nbootstraps == 1){
    ...
    if(object$aic == TRUE){info = "AIC"}else{info = "BIC"}
    bic_legend = paste(info, "=", round(object$info.criterion,2))

    graphics::matplot(t(object$WCEmat), lty=1, type = 'l', ylab = 'weights',
                      xlab = 'Time elapsed')
    ...
    if(isTRUE(hist.covariates) & !is.null(object$covariates)){
      for (i in object$covariates) {
        ...
        graphics::hist(object$coef[,i],
                       main = paste0("Histogram of ", i," coefficient with ",
                                     object$nbootstraps,
    ...
    graphics::matplot((object$WCEmat[1,]), lty=1, type = 'l', ylab = 'weights',
                      xlab = 'Time elapsed')
    graphics::title(paste0('Estimated weight functions\n with confidence interval (',
                           object$nbootstraps," bootstraps)"))
    graphics::matplot((object$WCEmat[1,]), pch = 1, add = TRUE)
    graphics::matplot((object$WCEmat_CI[1,]),type = c("l"), lty = 2, col="red",
                      add = TRUE)
    graphics::matplot((object$WCEmat_CI[2,]),type = c("l"), lty = 2, col="red",
                      add = TRUE)
