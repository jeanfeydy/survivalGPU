...
coxphGPU.default <- function(formula, data, ties = c("efron", "breslow"),
  ...
  indx <- match(
    c(
      "formula", "data", "weights", "subset", "na.action",
      "cluster", "id", "istate"
    ),
    names(Call),
    nomatch = 0
  ...
  if (multi) {
    ...
    else covlist2 <- parsecovar2(covlist, statedata,
      dformula = dformula,
      Terms, transitions, states
    )
    tmap <- covlist2$tmap
  ...
  if (sum(Y[, ncol(Y)]) == 0) {
    ...
    concordance = c(
      concordant = 0, discordant = 0, tied.x = 0, tied.y = 0, tied.xy = 0,
  ...
  if (is.character(fit)) {
    ...
    else temp <- apply(fit$linear.predictors, 2, concordancefit,
      y = Y,
