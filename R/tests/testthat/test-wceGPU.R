...
# Original WCE
wce <- WCE::WCE(data = drugdata, analysis =  "Cox", nknots =  1, cutoff =  90,
...
test_that("WCE mat", {
  ...
  expect_equal(round(as.vector(wce$WCEmat),6),
...
test_that("coef covariates", {
  ...
  expect_equal(round(as.vector(wce$beta.hat.covariates),3),
...
test_that("SE covariates", {
  ...
  expect_equal(round(as.vector(wce$se.covariates),3),
...
test_that("Vcovmat", {
  ...
  expect_equal(round(wce$vcovmat[[1]],6),
...
test_that("Partial ll", {
  ...
  expect_equal(round(as.vector(wce$loglik),3),
...
test_that("info.criterion", {
  ...
  expect_equal(round(as.numeric(wce$info.criterion),0),
